...
describe('GET /todo/:userId', () => {
    it('should get todos by user id', async () => {
        ...
        const res = await request(app).get('/todo/888').set('Authorization', `Bearer ${TOKEN}`);

        expect(res.statusCode).toBe(200);
        expect(res.body).toStrictEqual(todo1);
        expect(makeRequest).toHaveBeenCalledTimes(1);
        expect(makeRequest).toHaveBeenCalledWith(
            `${REST_API_URL}/users/888/todos`,
            'GET',
            `Bearer ${TOKEN}`,
        );
    });
    ...
    it('should handle errors getting users by id', async () => {
        ...
        const res = await request(app).get('/todo/2').set('Authorization', `Bearer ${TOKEN}`);

        expect(res.statusCode).toBe(500);
        expect(res.text).toBe('Error getting todos');
        expect(makeRequest).toHaveBeenCalledTimes(1);
        expect(makeRequest).toHaveBeenCalledWith(
            `${REST_API_URL}/users/2/todos`,
            'GET',
            `Bearer ${TOKEN}`,
        );
    });
